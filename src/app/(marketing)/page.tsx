...
const STEPS = [
  ...
  },
  {
    ...
    body: "Take a ZIP of plain HTML, CSS and JavaScript, or push it live to a URL you choose in about twenty seconds.",
  },
];
...
const FEATURES = [
  ...
  },
  {
    ...
    body: "Templates with about and contact pages stay multi-page all the way through export and deploy.",
  },
  {
    icon: ShieldCheck,
  ...
  },
  {
    ...
    body: "Pro projects publish to a global edge network under a name you choose, and redeploy in place when you edit.",
    wide: true,
  },
];
...
const FAQ = [
  ...
  },
  {
    ...
    a: "Free gives you one project, the full editor, unlimited previews and a single download. Pro removes every limit and adds one-click deployment with your own project name.",
  },
  {
    ...
    <>
      ...
      <section className="relative overflow-hidden">
        <div
          ...
          style={{
            background:
              "radial-gradient(900px 420px at 50% -10%, rgba(41,151,255,0.20), transparent 70%)",
          }}
          aria-hidden="true"
        />
        ...
        <div className="container-page relative pb-16 pt-20 sm:pt-28">
          ...
          <div className="grid items-end gap-10 lg:grid-cols-[1.35fr_1fr] lg:gap-16">
            <h1 className="text-balance-tight animate-fade-up text-[clamp(2.7rem,6.4vw,5.1rem)] font-semibold leading-[0.98]">
              ...
              <span className="text-white/35">Ship it.</span>
            </h1>

            <div className="animate-fade-up [animation-delay:120ms]">
              <p className="max-w-[46ch] text-[17px] leading-relaxed text-white/55">
                A marketplace of production-ready templates, a visual editor that works by clicking
                the page, and a one-click path to a live URL. The files stay plain HTML, CSS and
                JavaScript — yours to take anywhere.
              </p>
              ...
              <p className="mt-4 text-[13px] text-white/30">
                Free plan, no card required. One website and one download included.
              </p>
            </div>
          </div>
        </div>
      ...
      <section className="border-y border-white/[0.07] bg-white/[0.015]">
        <div className="container-page py-6">
          <div className="scrollbar-none mask-fade-x flex gap-2 overflow-x-auto">
            {categories.slice(0, 16).map((category) => (
              <Link
                key={category.slug}
                href={`/templates?category=${category.slug}`}
                className="shrink-0 rounded-full border border-white/[0.07] px-3.5 py-1.5 text-[13px] text-white/45 transition-all duration-300 hover:border-white/20 hover:text-white"
              >
                {category.name}
                <span className="ml-1.5 text-white/25">{category.count}</span>
              </Link>
            ))}
          </div>
        </div>
      ...
      <section className="container-page py-20 sm:py-28" id="showcase">
        <div className="mb-10 flex flex-wrap items-end justify-between gap-6">
          <div>
            <p className="mb-3 text-[13px] font-medium uppercase tracking-[0.16em] text-white/30">
      ...
      {/* ─────────────────────────────────────────────────── how it works */}
      <section id="how" className="border-t border-white/[0.07] py-20 sm:py-28">
        <div className="container-page">
          ...
          <ol className="grid gap-px overflow-hidden rounded-[20px] border border-white/[0.07] bg-white/[0.06] md:grid-cols-2 lg:grid-cols-4">
            {STEPS.map((step, index) => (
              <li key={step.title} className="group bg-black p-7 transition-colors hover:bg-white/[0.02]">
                <div className="flex items-center justify-between">
                  <span className="flex size-9 items-center justify-center rounded-xl bg-white/[0.06] text-white/70 transition-colors group-hover:bg-white/10 group-hover:text-white">
      ...
      {/* ───────────────────────────────────────────────────────── features */}
      <section id="features" className="border-t border-white/[0.07] py-20 sm:py-28">
        <div className="container-page">
          <div className="mb-14 max-w-[52ch]">
            <p className="mb-3 text-[13px] font-medium uppercase tracking-[0.16em] text-white/30">
              What you get
          ...
          <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
            {FEATURES.map((feature) => (
              <article
                key={feature.title}
                className={`card-surface group rounded-[20px] p-7 transition-all duration-500 hover:border-white/20 ${
                  feature.wide ? "lg:col-span-2" : ""
                }`}
              >
                <span className="flex size-10 items-center justify-center rounded-xl bg-white/[0.06] text-white/70 transition-colors group-hover:text-white">
                  <feature.icon className="size-[18px]" />
                </span>
                <h3 className="mt-6 text-[16px] font-medium">{feature.title}</h3>
                <p className="mt-2 max-w-[54ch] text-[14px] leading-relaxed text-white/45">
                  {feature.body}
                </p>
              </article>
            ))}
      ...
      {/* ────────────────────────────────────────────────────────── pricing */}
      <section id="pricing" className="border-t border-white/[0.07] py-20 sm:py-28">
        <div className="container-page">
      ...
      {/* ────────────────────────────────────────────────────────────── faq */}
      <section id="faq" className="border-t border-white/[0.07] py-20 sm:py-28">
          ...
          <div className="divide-y divide-white/[0.07] border-y border-white/[0.07]">
            {FAQ.map((item) => (
              <details key={item.q} className="group py-5">
                <summary className="flex cursor-pointer list-none items-center justify-between gap-6 text-[15px] font-medium text-white/85 transition-colors hover:text-white">
                  {item.q}
                ...
                <p className="mt-3 max-w-[66ch] text-[14px] leading-relaxed text-white/45">{item.a}</p>
              </details>
            ))}
          </div>
        </div>
      ...
      <section className="border-t border-white/[0.07]">
        <div className="container-page py-24 sm:py-32">
          <div className="relative overflow-hidden rounded-[28px] border border-white/[0.09] bg-gradient-to-b from-white/[0.05] to-transparent px-8 py-16 text-center sm:px-16">
            <div
              ...
              aria-hidden="true"
            />
            <Sparkles className="mx-auto size-5 text-white/30" />
            ...
            <p className="mx-auto mt-5 max-w-[48ch] text-[15px] leading-relaxed text-white/50">
