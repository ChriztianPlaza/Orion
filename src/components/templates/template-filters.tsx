...
export function TemplateFilters({
  ...
  return (
    <div className="space-y-5">
      <div className="flex flex-wrap items-center gap-2.5">
        <div className="relative min-w-[240px] flex-1">
          ...
          {search && (
            <button
              onClick={() => setSearch("")}
              className="absolute right-3 top-1/2 -translate-y-1/2 rounded p-1 text-white/30 hover:text-white"
      ...
      {showAdvanced && (
        <div className="flex flex-wrap items-center gap-2.5 rounded-2xl border border-white/[0.07] bg-white/[0.02] p-3">
          <Select
            value={activeTier}
      ...
      <div className="scrollbar-none mask-fade-x -mx-1 flex gap-2 overflow-x-auto px-1 pb-1">
        <CategoryChip
          label="All"
    ...
    <button
      ...
      className={cn(
        ...
        active
          ...
          : "border-white/[0.07] text-white/50 hover:border-white/20 hover:text-white",
      )}
    >
      {label}
      <span className={cn("ml-1.5", active ? "text-black/40" : "text-white/25")}>{count}</span>
    </button>
  );
}
